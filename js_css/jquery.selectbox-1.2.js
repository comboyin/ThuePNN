...
jQuery.SelectBox = function(selectobj, options) {
	...
	opt.maxHeight = opt.maxHeight || 300; // max height of dropdown list
	...
	.focus(function(){
		if ($container.not(':visible')) {
			inFocus = true;
			jQuery("div.selectbox-wrapper").hide();
			$container.show();
		}
	})
	...
	function setupContainer(options) {		
		...
		$container.html('<div class="header-selectbox-wapper-1"><div class="header-selectbox-wapper-2"></div></div>');
		return $container;
	}
	...
	function setCurrent() {	
		...
		else
		{
			...
			selectStr = jQuery(li).html().split("&nbsp;");
			$input.val(selectStr[0]);
		}
		return true;
	}
	...
	function getSelectOptions(parentid) {
		...
		var counter = select_options.length;
		...
		} else select_options.each(function(index) {
			var li = document.createElement('li');
			li.setAttribute('id', parentid + '_' + jQuery(this).val());
			li.innerHTML = jQuery(this).html()+"&nbsp;";
			if (jQuery(this).is(':selected')) {
    ...
    jQuery(document).bind("mousedown", function (e) {
        if(inFocus && !jQuery.contains($container.get(0), e.target) && jQuery(e.target).hasClass('selectbox-wrapper') == false) {
            hideMe();
        }
    });
};
